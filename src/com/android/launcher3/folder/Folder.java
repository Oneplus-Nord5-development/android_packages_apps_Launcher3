...
public class Folder extends AbstractFloatingView implements ClipPathView, DragSource,
        ...
        LauncherBindableItemsContainer, LauncherPrefChangeListener {
    ...
    private void animateOpen(List<ItemInfo> items, int pageNo) {
        ...
        // Hide the folder icon before skipping the first frame to avoid blur flicker.
        if (mFolderIcon != null) {
            mFolderIcon.setIconVisible(false);
            mFolderIcon.drawLeaveBehindIfExists();
        }
    ...
    public int getPreviewBackgroundColor() {
        int color = Themes.getAttrColor(getContext(), R.attr.folderPreviewColor);
        return isBackgroundBlurEnabled()
                ? ColorUtils.setAlphaComponent(color, Math.min(160, android.graphics.Color.alpha(color)))
                : color;
    }

    public int getFolderBackgroundColor() {
        int color = Themes.getAttrColor(getContext(), R.attr.folderBackgroundColor);
        return isBackgroundBlurEnabled()
                ? ColorUtils.setAlphaComponent(color, Math.min(120, android.graphics.Color.alpha(color)))
                : color;
    }
    ...
    private boolean shouldShowBackgroundBlur() {
        if (!isAttachedToWindow() || !isShown() || !hasWindowFocus() || !isBackgroundBlurEnabled()) {
            return false;
        }
        return true;
    }
    ...
    private boolean ensureBackgroundBlurDrawable() {
        ...
        mBackgroundBlurDrawable.setColor(android.graphics.Color.TRANSPARENT);
    ...
    private void drawBackgroundBlur(Canvas canvas) {
        if (!ensureBackgroundBlurDrawable()) {
            return;
        }

        mBackgroundBlurBounds.set(0, 0, getWidth(), getHeight());
        if (!mBackgroundBlurBounds.equals(mBackgroundBlurDrawable.getBounds())) {
            mBackgroundBlurDrawable.setBounds(mBackgroundBlurBounds);
        }
        int roundedCornerRadius = Math.round(mBackground.getCornerRadius());
        if (roundedCornerRadius != mLastBackgroundBlurRadius) {
            mBackgroundBlurDrawable.setCornerRadius(mBackground.getCornerRadius());
